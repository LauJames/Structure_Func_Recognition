...
tf.flags.DEFINE_string("train_data_file", "./data/section3500",
                       "Data source for the train data.")
tf.flags.DEFINE_string("test_data_file", "./data/section500",
                       "Data source for the test data.")
tf.flags.DEFINE_string("tensorboard_dir", "tensorboard_dir/textCNN_section", "saving path of tensorboard")
tf.flags.DEFINE_string("save_dir", "checkpoints/textCNN_section", "save base dir")
...
tf.flags.DEFINE_float("learning_rate", 0.0001, "learning rate (default:1e-3)")
...
def train():
    ...
    x_train, y_train, x_dev, y_dev = dataHelper.load_section_data(FLAGS.train_data_file, FLAGS.dev_sample_percentage, FLAGS.save_dir, FLAGS.seq_length)
...
def test():
    ...
    start_time = time.time()
    x_raw, y_test = dataHelper.get_section(FLAGS.test_data_file)
...
if __name__ == '__main__':
    if len(sys.argv) != 2 or sys.argv[1] not in ['train', 'test']:
        raise ValueError("Please input: python3 runCNN_section.py [train/test]")
